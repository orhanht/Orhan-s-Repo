...
class CircularBuffer:
    def __init__(self, capacity):
        ...
        self.buffer_list = []
        self.read_data_index = -1

    def read(self):
        if len(self.buffer_list) == self.read_data_index + 1:
            raise BufferEmptyException("Buffer is empty!")
        else:
            self.read_data_index += 1
            return self.buffer_list[self.read_data_index]

    def write(self, data):
        if len(self.buffer_list) == self.capacity and self.read_data_index < 0:
            raise BufferFullException("Buffer is full!")
        elif len(self.buffer_list) < self.capacity:
            self.buffer_list.append(data)
        elif len(self.buffer_list) == self.capacity and self.read_data_index >= 0:
            self.overwrite(data)

    def overwrite(self, data):
        if len(self.buffer_list) == self.capacity:
            if self.read_data_index >= 0:
                self.read_data_index -= 1
