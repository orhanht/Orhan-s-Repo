...
class Allergies(object):
    ...
    def lst(self):
        list_of_allergies = []
        poss_allergies_dict = {"cats": 128, "pollen": 64, "chocolate": 32, "tomatoes": 16, "strawberries": 8,
                               "shellfish": 4, "peanuts": 2, "eggs": 1}
        while self.score >= 256:
            self.score -= 256
        for i in poss_allergies_dict.keys():
            if self.score >= poss_allergies_dict[i]:
                self.score -= poss_allergies_dict[i]
                list_of_allergies.append(i)
            if self.score == 0:
                break
